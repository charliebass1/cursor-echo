...
Observed in data: This coding pass used 30 retained non-grounding evidence rows from Output A: 12 Cursor Forum rows, 10 GitHub rows, and 8 YouTube transcript/search rows. Reddit and X produced no retained rows under access limits. Cursor docs/blog/changelog rows were used only as product grounding, not as user pain evidence.
...
- GitHub/search-only issue rows point to terminal/tool execution failures and mode regressions: [cursor/cursor issue #3853](https://github.com/cursor/cursor/issues/3853), [cursor/cursor issue #3730](https://github.com/cursor/cursor/issues/3730).
...
- GitHub/search-only issue rows point to rules acknowledged then violated or not passed into context: [cursor/cursor issue #3565](https://github.com/cursor/cursor/issues/3565), [cursor/cursor issue #3706](https://github.com/cursor/cursor/issues/3706).
...
- Forum and YouTube rows also show handoff friction: [Agent Finish Steals Focus](https://forum.cursor.com/t/agent-finish-steals-focus/157090), [First look at Cursor background agents](https://www.youtube.com/watch?v=Mu3J-odJyb4).
...
High evidence strength / Lower or mixed Cursor-actionability:
- T2 Context And Instruction Retention
...
Method: Re-coded 6 of 30 retained non-grounding items, a 20% internal consistency sample. This is not inter-rater reliability because the same agent performed both passes; it is only a check that the theme definitions are stable enough to proceed.
